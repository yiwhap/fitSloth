...
def main():
    ...
    for seed in seeds:
        ...
        if args.evaluate_only:
            config = json.loads((out / 'training.json').read_text())
    ...
    for seed in reversed(seeds):
        ...
        summary = json.loads((comparison.out / 'summary.json').read_text())
        run = dict(train_seed=seed, best_epoch=json.loads((out / 'training.json').read_text())['best_epoch'],
